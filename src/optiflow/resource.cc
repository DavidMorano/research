...
/* Update the operand latency  of a resource descriptor, called when a
 * parameter overwrites the default value. Needs to be called before the
 * pool is created. The function sim_check_options is a good place to call
 * this function. */
void
res_update_oplat(struct res_desc *pool, int ndesc, int class, int oplat)
{
  int i, j;

  for (i=0; i<ndesc; i++) {
    for (j=0; j<MAX_RES_CLASSES && pool[i].x[j].class; j++) {
      if (pool[i].x[j].class == class) {
	pool[i].x[j].oplat = oplat;
      }
    }
  }
}

/* Update the issue latency  of a resource descriptor, called when a
 * parameter overwrites the default value. Needs to be called before the
 * pool is created. The function sim_check_options is a good place to call
 * this function. */
void
res_update_issuelat(struct res_desc *pool, int ndesc, int class, int issuelat)
{
  int i, j;
...
res_dump(struct res_pool *pool, FILE *stream)
{
  ...
  fprintf(stream, "Resource pool: %s", pool->name);
