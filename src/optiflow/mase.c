...
int sim_main(void)
{
  ...
  if (fastfwd_count > 0) {
     ...
     * mode. */
    counter_t	icount;			/* count of fastfwd instrs */
    ...
    fprintf(stderr, "sim: ** fast forwarding %llu insts **\n", 
		((sqword_t) fastfwd_count)) ;

	{
		char	digbuf[DIGBUFLEN + 1] ;


		ctdecull(digbuf,DIGBUFLEN,fastfwd_count) ;

    		fprintf(stderr, 
		"sim: ** fast forwarding %s insts **\n", digbuf) ;
