...
#define	DAM	1		/* this means that we fix certain things */
...
/* aking the inverse (one's compliment) of an address */

/****

We use the macro 'INVADDR' below to take the one's compliment of
an address.  This should work with either an ANSI or "correct" (non-ANSI)
compiler.

Question: Whom are we trying to protect by do this operation below?
The running benchmark program or the simulator (us) itself?

Answer: Us!  Not the target benchmark program.

So we only align target program mememory addresses to a multiple of
4 bytes, even though the target benchmark program might do 8-byte
(alighned) operations.  This is because the host machine (SPARC)
requires only 4-byte alighnments, even for 8-byte aligned operations.

****/

#ifndef	INVADDR
#define	INVADDR(a)	(~ ((md_addr_t) (a)))
#else
#error	"multiple definition of INVADDR"
#endif
...
#define MEM_READ_HALF(MEM, ADDR)	\
		MD_SWAPH(MEM_READ(MEM, ((ADDR) & INVADDR(1)), half_t))
#define MEM_READ_SHALF(MEM, ADDR)	\
		MD_SWAPH(MEM_READ(MEM, ((ADDR) & INVADDR(1)), shalf_t))
#define MEM_READ_WORD(MEM, ADDR)	\
		MD_SWAPW(MEM_READ(MEM, ((ADDR) & INVADDR(3)), word_t))
#define MEM_READ_SWORD(MEM, ADDR)	\
		MD_SWAPW(MEM_READ(MEM, ((ADDR) & INVADDR(3)), sword_t))

#ifdef HOST_HAS_QWORD
#define MEM_READ_QWORD(MEM, ADDR)	\
		MD_SWAPQ(MEM_READ(MEM, ((ADDR) & INVADDR(3)), qword_t))
#define MEM_READ_SQWORD(MEM, ADDR)	\
		MD_SWAPQ(MEM_READ(MEM, ((ADDR) & INVADDR(3)), sqword_t))
...
#define MEM_WRITE_HALF(MEM, ADDR, VAL)					\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(1)), half_t, MD_SWAPH(VAL))
#define MEM_WRITE_SHALF(MEM, ADDR, VAL)					\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(1)), shalf_t, MD_SWAPH(VAL))
#define MEM_WRITE_WORD(MEM, ADDR, VAL)					\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), word_t, MD_SWAPW(VAL))
#define MEM_WRITE_SWORD(MEM, ADDR, VAL)					\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), sword_t, MD_SWAPW(VAL))

#define MEM_WRITE_SFLOAT(MEM, ADDR, VAL)				\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), sfloat_t, MD_SWAPW(VAL))

#define MEM_WRITE_DFLOAT(MEM, ADDR, VAL)				\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), dfloat_t, MD_SWAPQ(VAL))

#ifdef HOST_HAS_QWORD
#define MEM_WRITE_QWORD(MEM, ADDR, VAL)					\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), qword_t, MD_SWAPQ(VAL))
#define MEM_WRITE_SQWORD(MEM, ADDR, VAL)				\
		MEM_WRITE(MEM, ((ADDR) & INVADDR(3)), sqword_t, MD_SWAPQ(VAL))
#endif /* HOST_HAS_QWORD */
