/* levo-exec.c - sim-levo exec stage */
...
/* end subroutine (read_store_data) */
...
/* end subroutine (transfer_store_data) */
	    ...
	    int header)				/* print header? */
{
  ...
  if (re->completed) 
	...
	fprintf(stream, "ISSUED\n");
  else if (re->queued) 
	fprintf(stream, "READY (Scheduler delay: %d)\n",
      re->schedule_wait_time);
  else 
	fprintf(stream, "NOT READY\n");
...
/* end subroutine (eventq_queue_event) */
...
/* end subroutine (eventq_next_event) */
...
/* end subroutine (mpq_queue_event) */
...
/*check if the ranges of store and load are intersected*/
int is_intersection(md_addr_t str_addr, int str_size, 
	md_addr_t ld_addr, int ld_size)
{
  md_addr_t max_addr, min_addr;
  int size_max, size_min;

  /*idetify which one is greater*/
  max_addr = ld_addr; 
  size_max = ld_size;
  min_addr = str_addr;
  size_min = str_size;
...
void lsq_refresh(void)
{
       ...
       i++, index=(index + 1) % LSQ_size)
  {
	...
	/*else if the base addr doesn't match, check the case of partial forwarding*/
	else {
	  int intersected = is_intersection ( sinfo_table[j].addr, LSQ[sinfo_table[j].index].mem_size, load_addr, load_size); 
	  if (intersected==1) {
	    prevent_queue = 1;
	    break;
	  }
...
/*end subroutine (lsq_refresh) */


/*
 * MASE_ISSUE() - issue instructions to functional units
 *
 * Attempt to issue all operations in the ready queue. 
Instructions in the ready
 * queue have all register and memory dependencies satisfied. 
This function checks
 * to see if a function unit is available in this 
cycle to commence execution of
 * the operation. 
If all goes well, the function unit is allocated, a writeback
 * event is scheduled, and the instruction begins execution. */

void
levo_issue(void)
{
  unsigned int lat;			/* latency of operation */
  ...
  for (n_issued=0;
       node && n_issued < issue_width; node = next_node) {
    ...
    if (RELINK_VALID(node)) {
      ...
      /* instruction hasn't made it it execute yet -- 
	...
	continue;
      ...
      /* check for stores */
      if (re->in_LSQ && 
	((MD_OP_FLAGS(re->op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))) {

        /* Stores complete in effectively zero time. Result is written
         * into the LSQ. The actual store into the memory system occurs
         * when the instruction is retired. (See levo_commit()) */

        re->issued = TRUE;
        re->completed = TRUE;
        if ((re->odep_name[0] != DNA) || (re->odep_name[1] != DNA)) 
	  ...
	  /* scan the LSQ to see if recovery is necessary */
	    ...
	    if (((MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_STORE)) == 
		(F_MEM|F_STORE))) {

	      if (ISQ[LSQ[index].isq_index].mem_addr == 
		read_idep_list(re, DEP_NAME(DTMP)).addr)
		break;

	    } else if (((MD_OP_FLAGS(LSQ[index].op) & (F_MEM|F_LOAD)) == 
		(F_MEM|F_LOAD))) {
		...
		int transfer_ok;
	        ...
	        /* Transfer the store data to the load. 
		If the transfer was successful,
	         * the instruction will need to be re-executed 
		if has been already
	         * issued.  The re-execution is necessary 
		in order to get the correct
	         * value into the destination register of the load.  
		If the transfer
	         * was not successful, there is a partial store.  
		The instruction is
	...
	/* check for loads so they access the cache */
	  ...
	  /* For load instructions, we need to determine the cache access 
	latency.
	   ...
	   * latency.  
	A mem_req_table entry is created that the callback function
	   * uses to add it to the event queue when the latency is 
	determined.  No
	   * entry is created if the requested matched in the LSQ 
	since the latency
	   * is automatically one.
	   *
	   * MEM_KNOWN (with latency): The memory system returns a 
	latency of the
	   * operation.  This latency is used to schedule the writeback 
	event unless
	  ...
	  if (cache_dl1 && valid_addr && !prevent_issue && !re->dtlb_miss) {
	    ...
	    status = levo_cache_access(cache_dl1,
	      Read, (mem_addr & ~(re->mem_size-1)), NULL, re->mem_size,
	      sim_cycle, NULL, NULL, load_cb, req_id, &lat
            );
	    ...
	    if ((status == MEM_RETRY) && !re->match_in_LSQ) {
	      ...
	      if ((lat > cache_dl1_lat) || (status == MEM_UNKNOWN)) 
