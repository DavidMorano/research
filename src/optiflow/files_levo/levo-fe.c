...
/* end subroutine (fe_dump) */


/*  DECODE AND DISPATCH STAGE - 
	decode instructions and allocate ROB and LSQ resources */
	      ...
	      int reg_name)			/* input register name */
{
  ...
  if (!rt_ent || !rt_ent->re) {
    ...
    if (REG_IS_FP(idep_name)) {
      ...
      case vt_dfloat:
        re->idep_value[idep_num].d = 
		regs.regs_F.d[FP_REG_INDEX(idep_name) >> 1];
        break;
  ...
  } else if (rt_ent->re->odep_valid[rt_ent->odep_num]) {	

/* active creator - completed */
	...
	fatal("Invalid type in rob_link_idep");
      ...
      re->idep_value[idep_num].sw = 
		rt_ent->re->odep_value[rt_ent->odep_num].sw;
...
/* end subroutine (rob_install_odep) */
...
levo_dispatch()
{
  ...
  while (
	...
	bprintf(pip->ofp,"id> ia=%016llx %016llx \n",
		dp_PC,inst) ;
	bprintf(pip->ofp,"	i1=%d i2=%d i3=%d o1=%d o2=%d\n",
		in1,in2,in3,out1,out2) ;

		rs = md_disassemble(inst,dp_PC,disbuf,100) ;
    ...
    if ((MD_OP_FLAGS(op) & F_MEM) && (LSQ_num == LSQ_size)) 
	break;
    ...
    /* 
		...
		Check to see if the branch is predicted taken, but the
		predicted target doesn't match the computed target
		(i.e., misfetch).  In this case, update the fetch PC
...
*/
        ...
        && (target_addr != dp_pred_NPC) && br_pred_taken)
    {
      ...
      /* 
		...
		If there is a misfetch, the target address is used as
		the NPC.  This is either the correct NPC if the branch
		was supposed to be taken or a different incorrect NPC
		if the branch was supposed to be not taken.  In the
		former case, the flag misfetch_only is set to prevent a
		full-fledge branch misprediction.  In the latter, there
		is a misfetch now and a branch misprediction later.
...
fetch_dump(FILE *stream)			/* output stream */
{
  ...
  if (icache_block)
    ...
    fprintf(stream, 
	"fetch blocked due to icache miss (lat: %d)\n", icache_delay);

}
/* end subroutine (fetch_dump) */
...
levo_fetch(void)
{
  unsigned int lat;			/* latency of TLB/cache access */
    ...
    i++)
  {
	...
	return;
      ...
      /* access the TLB */
      if (itlb) {
        status = levo_cache_access(itlb, Read, 
      ...
      if (cache_il1) {
        status = levo_cache_access(cache_il1, Read, 
          IACOMPRESS(fetch_PC), NULL, ISCOMPRESS(sizeof(md_inst_t)),
          sim_cycle, NULL, NULL, fetch_cb, 0, &lat);
...
/* fetch PC is bogus, send an invalid instruction (NOP) down the pipeline */
    ...
    if (pred_perfect) {
      ...
      if (MD_OP_FLAGS(op) & F_CTRL) {
        ...
        if (pu_regs.regs_NPC != pu_regs.regs_PC + sizeof(md_inst_t)) 
		branch_cnt++;
