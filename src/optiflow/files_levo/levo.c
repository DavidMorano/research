/* levo.c - micro architecture simulation environment */
/* LEVO version of MASE */
...
#ifdef	COMMENT
	rs = bprintf(pip->ofp,"I can print something inside sim\n") ;

#if	F_DEBUGS
	eprintf("sim_init: bprintf() rs=%d \n",rs) ;
#endif
              ...
              char **envp)		/* program environment */
{
...
#if	F_DEBUGS
	eprintf("sim_load_prog: entered ofp=%p magic=%08lx (%lu)\n",
		pi.ofp,
		pi.ofp->magic,
		pi.ofp->magic) ;
#endif
...
#if	F_DEBUGS
	eprintf("sim_load_prog: magic=%08lx\n",pi.ofp->magic) ;
#endif
  ...
  dlite_init(levo_reg_obj, levo_mem_obj, levo_mstate_obj);
...
#if	F_DEBUGS
	eprintf("sim_main: entered ofp=%p u_fstat() rs=%d\n",
		pip->ofp,u_fstat(4,&sb)) ;
#endif

#ifdef	COMMENT
	rs = bprintf(pip->ofp,"hello from sim_main\n") ;

#if	F_DEBUGS
	eprintf("sim_main: bprintf() rs=%d\n",rs) ;
#endif
...
/* 
	Execute the instructions.  The pre-update registers are used
	since the macros are already written using this file and
	since everything is in-order and non-speculative, only one
...
#include "machine.def"
      ...
      if (dlite_check_break(pu_regs.regs_NPC,
        ...
        dlite_main(pu_regs.regs_PC, pu_regs.regs_NPC, 
		sim_num_insn, &pu_regs, mem);
...
/* Now that fastforwarding is complete, make sure both register 
files are the same. */
  ...
  for (;;) {
    ...
    if (((ROB_head + ROB_num) % ROB_size) != ROB_tail) 
  ...
  } /* end for (main simulation loop) */

	if (pip->ofp != NULL)
	bprintf(pip->ofp,"reached the end of 'sim_main()' !\n") ;
