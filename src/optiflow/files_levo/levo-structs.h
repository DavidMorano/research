/* levo-structs.h - sim-mase data structure definitions */
...
#ifndef	LEVOSTRUCTS_INCLUDE
#define	LEVOSTRUCTS_INCLUDE		1
...
struct inst_state {
  ...
  int is_write;				/* set if write (store) */
		/* array of output dependencies and values */
  struct reg_state_t reg_state[MAX_ODEPS]; 
} ;


/* 

	A record that transports info from fetch to dispatch.  Contains
	some information about the instruction including decode
...
struct fetch_rec {
  ...
  int mem_size;				/* size of memory operation */
	int	op ;			/* "op" from fetch pre-decode */
} ;


/* 

	A rename table entry vector maps a logical register to
	a creator in the ROB and a specific output operand.
...
/* chains to consuming operations */
  struct RE_link *odep_list[MAX_ODEPS];        
