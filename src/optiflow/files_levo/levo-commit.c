/* levo-commit.c - sim-levo commit stage */
...
levo_commit(void)
{
  unsigned int lat;				/* latency of store */
  ...
  while (ROB_num > 0 && committed < commit_width) {
	...
	if (fu) {
	  ...
	  /* access the D-TLB */
	  if (dtlb) {
	    enum mem_status status = levo_cache_access(
      ...
      for (i=0; i<MAX_ODEPS; i++) {
        ...
        rename_remove_entry(LSQ[LSQ_head].odep_rename[i], 
		LSQ[LSQ_head].odep_name[i]);
    ...
    if (checker_commit(re, lsq_chk_error)) 
	events |= PEV_CHKERROR;
  ...
  } /* end while (trying to retire ROB entries) */
 ...
 * ROB[recover_index]. */
/* index of instruction invoking recovery */
void
rob_recover(int recover_index)		
{
  int i; 
...
levo_writeback(void)
{
  ...
  while ((re = eventq_next_event())) {
    ...
    /* For DTLB misses, 
	place instruction back on ready queue for cache access. */
    if (re->dtlb_miss) {
      re->issued = FALSE;
      readyq_enqueue(re);
      continue;
