	      ...
	      md_addr_t addr)		/* virtual address to translate */
{
  ...
  for (prev=NULL, pte=mem->ptab[MEM_PTAB_SET(addr)];
	  ...
	  if (prev) {
	      ...
	      prev->next = pte->next;
		set = MEM_PTAB_SET(addr) ;
	      pte->next = mem->ptab[set] ;
	      mem->ptab[set] = pte ;
	    }
	  return pte->page;
	}
...
/* end subroutine (mem_check_fault) */
...
/* dupplicate one memory space to another */
int mem_duplicate(m2p,mp)
struct mem_t	*m2p, *mp ;
{
  	struct mem_pte_t	*pp, *npp ;
	...
	for (i = 0 ; i < MEM_PTAB_SIZE ; i += 1) {
		...
		if (pp->page != NULL) {
		    ...
		    va = MEM_PTE_ADDR(pp,i) ;

		    fop = (caddr_t) MEM_PAGE(mp,va) ;

/* write something to insure that page is (now) present */

  		    MEM_WRITE_WORD(m2p,va,0) ;

/* translate VA to get PA of page */

		    top = mem_translate(m2p,va) ;

/* copy the page (quickly) */

		    memcpy(top,fop,MD_PAGE_SIZE) ;
