...
sim-mase dynamic checker and oracle */


#define	F_CHECKERROR		0
#define	F_CHECKERRECOVER	1
#define	F_RECOVERROB		0
#define	F_RECOVERFE		0
...
/* queue of instruction state data (oracle) */
struct inst_state *ISQ;			
...
/* 
Reads or writes from pre_update memory.  
For loads, if a memory location is not
...
/* end subroutine (release_hashtable_line) */
...
/* end subroutine (commit_reg_state) */
...
checker_exec(md_addr_t fetch_PC, struct fetch_rec *IFQ)
{
  ...
  if (is_trap) 
	fatal("Cannot call checker_exec until trap is committed.\n");

  if (is_syscall) 
	fatal("Cannot call checker_exec until syscall is committed.\n");
  ...
  /* Move the pu_regs.regs_NPC into pu_regs.regs_PC.  
	If it differs from the fetch PC, it
  ...
  /* Update stats.  
	The "total" stats are updated here while the "committed" stats
  ...
  /* Execution of syscalls are delayed until the instruction commits.  
	As a result,
...
checker_recover(int rollback_index)
{
  ...
  for (isq_index = ISQ_head; 
	!found_match; isq_index = (isq_index + 1) % ISQ_size) {
...
/* end subroutine (checker_recover) */
...
/* end subroutine (process_timing_dep) */
...
checker_print_errors(struct ROB_entry *re, struct inst_state *isq)
{
  ...
  if (re->NPC != isq->NPC) {
    myfprintf(stderr,
      "CHECKER ERROR! re NPC 0x%08p differs from "
	"oracle NPC 0x%08p (cycle %d)\n",
  ...
  if (re->fault != isq->fault) {
    myfprintf(stderr,
      "CHECKER ERROR! re fault %d differs from "
	"oracle fault %d at 0x%08p (cycle %d)\n",
...
/* end subroutine (checker_print_errors) */
...
checker_check(struct ROB_entry *re, struct inst_state *isq)
{
  ...
  if (re->timing_dep && found_error) 
	process_timing_dep(re,isq);
  else if (print_chk_err && found_error) 
	checker_print_errors(re, isq);
 ...
 * the oracle data.  If a mismatch occurs, the pipeline is flushed
...
checker_commit(struct ROB_entry *re, int lsq_chk_error)
{
  ...
  if (ISQ[ISQ_head].fault != md_fault_none)
    fatal("non-spec fault (%d) detected @ 0x%08p", 
	ISQ[ISQ_head].fault, ISQ[ISQ_head].PC);

  /* update stats */
  sim_num_insn++;
  if (MD_OP_FLAGS(ISQ[ISQ_head].op) & F_CTRL) 
	sim_num_branches += 1 ;
  ...
  if (ISQ[ISQ_head].is_syscall) {
    ...
    mem_access(mem, Write, ISQ[ISQ_head].mem_addr, 
	&(ISQ[ISQ_head].mem_value), ISQ[ISQ_head].mem_size);
  ...
  if (found_error) {
    ...
    if (stop_on_chk_err) 
	fatal("Aborting simulation due to checker error!");

    /* recover and restart with the next instruction */
