...
/* dnamic predicate related stuff (DAM) */

#define	MAXPREDTARGETS	4
...
/* a register update unit (RUU) station, this record is contained in the
   processors RUU, which serves as a collection of ordered reservation
   ...
   machine, it and its instruction's value is retired to the architectural
...
/* added by Dave Morano (DAM) for snooping & multipath */
...
static struct las	*lases ;	/* same number as RUUs */
...
/* RUUs */
	...
	size = RUU_size * sizeof(struct RUU_station) ;
...
/* LASes */
	...
	size = RUU_size * sizeof(struct las) ;
	lases = calloc(RUU_size,sizeof(struct las)) ;
...
/* here are the pipeline operations */
