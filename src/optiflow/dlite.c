  ...
  if (stat)
	...
	case sc_qword:
	  ...
	  break;
        case sc_sqword:
          val.type = et_sqword;
          val.value.as_sqword = *stat->variant.for_sqword.var;
          break;
	   ...
	   struct mem_t *mem)			/* memory to access */
{
  ...
  if (nargs == 1)
	...
	return "address argument must be an integral type";
      ...
      err_str = f_dlite_reg_obj(regs, /* is_write */TRUE, rt_PC, 0, &val);
      if (err_str != NULL) return err_str;
	    ...
	    struct mem_t *mem)			/* memory to access */
{
  ...
  myfprintf(stdout, "PC  (address of last committed instruction): 0x%08p\n",
	    regs->regs_PC);
  myfprintf(stdout, "NPC (address of next instruction to commit): 0x%08p\n",
	    regs->regs_NPC);
