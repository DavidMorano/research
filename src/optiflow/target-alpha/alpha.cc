...
/* end subroutine (md_init_decoder) */
...
/* end subroutine (md_print_insn) */


/* disassemble an Alpha instruction */
int md_disassemble(inst, pc, buf,buflen)
md_inst_t	inst ;
md_addr_t	pc ;
char		buf[] ;
int		buflen ;
{
  enum md_opcode op;
	...
	rs = sbuf_init(&buffer,buf,buflen) ;
  ...
  if (op <= OP_NA || op >= OP_MAX) {
      ...
      /* bogus instruction */
      sbuf_printf(&buffer, "<invalid %016llx>", inst) ;
	...
	cp = strwcpy(namebuf,MD_OP_NAME(op),10) ;

	if ((cp - namebuf) < 10)
		strwcpy(cp,"         ",(10 - (cp - namebuf))) ;

      /* FIXME: %-10s crashes on Suns!!! */
      sbuf_strw(&buffer,namebuf,10) ;
	...
	case 'a':
	  sbuf_printf(&buffer, "r%d", RA) ;

	  break;

	case 'b':
	  sbuf_printf(&buffer, "r%d", RB) ;

	  break;

	case 'c':
	  sbuf_printf(&buffer, "r%d", RC) ;

	  break;

	case 'A':
	  sbuf_printf(&buffer, "f%d", RA) ;

	  break;

	case 'B':
	  sbuf_printf(&buffer, "f%d", RB) ;

	  break;

	case 'C':
	  sbuf_printf(&buffer, "f%d", RC) ;

	  break;

	case 'o':
	  sbuf_printf(&buffer, "%d", (sword_t)SEXT(OFS)) ;

	  break;

	case 'j':
	  sbuf_printf(&buffer, "%016llx", pc + (SEXT(OFS) << 2) + 4) ;

	  break;

	case 'J':
	  sbuf_printf(&buffer, "%016llx", pc + (SEXT21(TARG) << 2) + 4) ;

	  break;

	case 'i':
	  sbuf_printf(&buffer,"%d", (word_t) IMM) ;
