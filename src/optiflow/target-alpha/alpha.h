...
/* Dave Morano (DAM) */
#define	CF_SWAPSUB	1		/* use a subroutine to swap */
...
typedef struct {
  ...
  qword_t tmp ;			/* (fake) temporary register */
} md_ctrl_t;
...
extern half_t			swap_half(half_t) ;
extern word_t			swap_word(word_t) ;
extern qword_t			swap_qword(qword_t) ;
...
#define SEXT(X)								\
  (((X) & 0x8000) ? ((sqword_t)(X) | \
	MKLL(0xffffffffffff0000)) : (sqword_t)(X))

#define SEXT21(X)							\
  (((X) & 0x100000) ? ((sqword_t)(X) | \
	MKLL(0xffffffffffe00000)) : (sqword_t)(X))

#define SEXT32(X)							\
  (((X) & 0x80000000) ? ((sqword_t)(X)| \
	MKLL(0xffffffff00000000)) : (sqword_t)(X))
...
#define IEEEFP_DBL_FRACTION(Q)	((Q) & MKULL(0xfffffffffffff))
