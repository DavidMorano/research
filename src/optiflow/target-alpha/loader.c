	     ...
	     int zero_bss_segs)		/* zero uninit data segment? */
{
...
#if	CF_DEBUGS && CF_LEVO
	eprintf("ld_load_prog: ofp=%p magic=%08lx (%lu)\n",
		pi.ofp,
		pi.ofp->magic,
		pi.ofp->magic) ;
#endif
  ...
  if (eio_valid(fname)) {
      ...
      if (sim_chkpt_fname != NULL)
	{
	  ...
	  myfprintf(stderr, "sim: fast forwarding to instruction %u\n",
		    restore_icnt);
...
#if	CF_DEBUGS && CF_LEVO
	eprintf("ld_load_prog: magic=%08lx\n",pi.ofp->magic) ;
#endif
...

  {
	...
	switch (MD_SWAPW(shdr.s_flags)) {
	  ...
	  case ECOFF_STYP_RTEXT:

#ifdef	DAM
		sizew = MD_SWAPQ(shdr.s_size) ;
	    p = calloc(sizew, sizeof(char));

	    if (!p)
	      fatal("out of virtual memory");

	    sizew = MD_SWAPQ(shdr.s_scnptr) ;
	    if (fseek(fobj, sizew, 0) == -1)
	      fatal("could not read `.text' from executable", i);

	    sizew = MD_SWAPQ(shdr.s_size) ;
	    if (fread(p, sizew, 1, fobj) < 1)
	      fatal("could not read text section from executable");

	    /* copy program section into simulator target memory */
	    iw = MD_SWAPQ(shdr.s_size) ;
	    mem_bcopy(mem_access, mem, Write,
		      MD_SWAPQ(shdr.s_vaddr), p, iw) ;
