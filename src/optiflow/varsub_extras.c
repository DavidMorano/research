...
/* Copyright © 1998,2001 David Morano.  All rights reserved. */

/******************************************************************************

	These routines operate on 'defined' and 'export'ed variables
	from a configuration file.  What we want to do is to perform a
	substitution on these variables from previous variables in the
	database or from the environment otherwise.  Variables that
	are already in the database get merged like in a path string
	with colon characters (':') separating the original values of
	the variables.
...
extern char	*getenv2(const char *,int) ;
extern char	*getenv3(const char *,int,char **) ;
...
VECSTR		*vsp ;
{
	...
	for (i = 0 ; vecstr_get(vsp,i,&sp) >= 0 ; i += 1) {
		...
		c += 1 ;
	        rs = varsub_add(op,esp,(cp - esp),vp,-1) ;
	        if (rs < 0)
	            break ;
...
int	s1len, s2len ;
{
	...
	rs = varsub_buf(var1p,s1,s1len,s2,s2len) ;
	slen = rs ;

	if ((var2p != NULL) && (rs <= 0)) {

	    rs = varsub_buf(var2p,s1,s1len,s2,s2len) ;
	    slen = rs ;
	}

#if	CF_DEBUGS
	debugprintf("varsub_subbuf: ret rs=%d slen=%d\n",rs,slen) ;
#endif
...
#if	CF_DEBUGS
	debugprintf("varsub_merge: entered\n") ;
#endif
...
/* do we have this particular value in this variable's value list already? */
	        ...
	        if (cmpvalue(sp,vp,vlen) == 0) {

#if	CF_DEBUGS
	            debugprintf("varsub_merge: already had this value\n") ;
#endif
...
#if	CF_DEBUGS
	                debugprintf("varsub_merge: found it \n") ;
#endif
...
#if	CF_DEBUGS
	debugprintf("varsub_merge: ret rs=%d\n",rs) ;
#endif /* CF_DEBUG */
...
static int cmpvalue(sp,vp,vlen)
const char	sp[] ;
const char	vp[] ;
int		vlen ;
{
...
#if	CF_DEBUGS
	debugprintf("var/cmpvalue: entered\n") ;
