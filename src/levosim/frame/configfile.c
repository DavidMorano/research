...
extern int	cfdeci(char *,int,int *), cfdecmfi(char *,int,int *) ;
...
/* initialize */
	...
	if ((rs = vecstr_init(&csp->defines,10,0)) < 0)
	    goto baddefines ;

	if ((rs = vecstr_init(&csp->unsets,10,0)) < 0)
	    goto badunsets ;

	if ((rs = vecstr_init(&csp->exports,10,0)) < 0)
	    goto badexports ;
...
/* check if key is a valid one, ignore invalid keys */
	    ...
	    if ((i = optmatch(configkeys,buf,fsb.flen)) < 0)
	        goto badkey ;
...
badconfig:
	csp->badline = line ;
	vecstr_free(&csp->exports) ;

badexports:
	vecstr_free(&csp->unsets) ;

badunsets:
	vecstr_free(&csp->defines) ;
