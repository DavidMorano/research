...
/******************************************************************************
	...
	for queue operations.  This type of queue can be used in shared
	memory areas that are mapped to different addresses in different
	address spaces!  That is the whole idea of this type of a queue.
	This means that all queue entries are stored with relative
	positions from the head of the queue.  This is done so that
	multiple processes (in different address spaces) can access the
	queue (once proper mutual exclusion is provided).

	This object CAN be used in applications where the object is
	moved (copied) in memory since no pointers (whatever) point
	back to the header.

	Important note:

	This type of queue (used for system OS or other executive type
	purposes) requires that all entries start with the structure
	'PLAINQ_ENT'.  This is necessary because that structure is
	used for the linking of the entries into the Q.  This is not
	a container object in the normal sense since access inside
	entries is required for operation!  If you want a container
	Q that does not have to access the entry object then pick
	some other Q object to use instead of this one!
...
int plainq_insgroup(qhp,gp,esize,n)
PLAINQ		*qhp ;
PLAINQ_ENT	*gp ;
int		esize, n ;
{
