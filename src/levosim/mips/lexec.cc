...
#define	F_DEBUGSMAIN	0
#define	F_EXPLICITSHIFT	0		/* explicit shift */
...
/* 01/11/06, Dave Morano, aid for double precision instructions */
struct divar {
	uint	hi, lo ;
} ;
...
#else
	eprintf("lexec: opexec=%d \n", ip->opexec) ;
	eprintf("lexec: s1=%08x s2=%08x s3=%08x s4=%08x s5=%08x\n",
		src1->ui,src2->ui,src3->ui,src4->ui,src5->ui) ;
	eprintf("lexec: cnst=%08x\n", cnst->ui) ;
#endif
...
#if	F_DEBUGS && 0
		eprintf("lexec: ADDIU src1=%08x cnst=%08x\n",
			src1->ui,cnst->ui) ;
#endif
...
#if F_DEBUGS && 0
		eprintf("lexec: ADDIU: %x + %x = %x\n",
			src1->ui,cnst->ui,(src1->ui+cnst->ui)) ;
		eprintf("lexec: dst1=%08x\n", dst1->ui) ;
#endif
...
#if F_DEBUGS && 0
		eprintf("lexec: BNE src1->i:%d src2->i:%d\n",src1->i,src2->i);
#endif
...
#if F_DEBUGS && 0
	eprintf("lexec: LEXECOP_JR (1) src2->i = %d  "
		"bcnd->i=%d  f_jump=%d\n",
			src2->i, *bcnd, *f_jump);
#endif
...
#ifdef	COMMENT
	eprintf("lexec:LEXECOP_JR (2) src2->i = %d  "
		"bcnd->i=%d  f_jump=%d\n",
...
#if	F_DEBUGS
	eprintf("lexec: DIVS s1=%08x s2=%08x\n",src1->ui,src2->ui) ;
#endif

	        dst1->f = src1->f / src2->f ;

#if	F_DEBUGS
	eprintf("lexec: DIVS d1=%08x \n",dst1->ui) ;
#endif
...
DATAVAL	*dst1 ;
{
	...
	eprintf("lexec/cmp: CMP %s(%d) instr=%08x\n",ct,tf,instr) ;
	}
#endif /* F_DEBUGS */
...
#if	F_DEBUGS
	eprintf("lexec/cmp: CMP SP FCR=%08x\n",src3->ui) ;
#endif
...
#if	F_DEBUGS
	eprintf("lexec/cmp: CMP SP unordered\n") ;
#endif
...
#if	F_DEBUGS
	eprintf("lexec/cmp: CMP DP FCR=%08x\n",src5->ui) ;
#endif
...
#if	F_DEBUGS
	eprintf("lexec/cmp: CMP done FCR=%08x\n",dst1->ui) ;
#endif
