...
/* do we already have this entry in the DB ? */
	    ...
	    f_already = FALSE ;
	    hdb_cursorinit(&mdp->dis,&cur) ;

	    while ((rs = hdb_fetch(&mdp->dis,key,&cur,&value)) >= 0) {

#if	F_DEBUGS
	        eprintf("mipsdis_disload: DIS DB rs=%d\n",rs) ;
#endif
...
#if	F_DEBUGS
	        eprintf("mipsdis_disload: hdb_store() rs=%d\n",rs) ;
#endif
...
#if	F_DEBUGS
	    eprintf("mipsdis_disload: INSTR DB rs=%d\n",rs) ;
#endif
...
#if	F_DEBUGS
	    eprintf("mipsdis_disload: ADDR DB rs=%d\n",rs) ;
#endif
