...
/* revision history:
   ...
   los operadores como complemento a 2.
	...
	instructions.  I also added the 'TRUE'/'FALSE' constants
	where they are used.  And I also change the branch conditional
...
int lexec(opcode, src1,src2,src3,src4,dst1,dst2,dst3,cnst,bcnd,mema,
		delay,ilptr,f_jump,f_nullify)
int	opcode ;
DATAVAL *src1, *src2, *src3, *src4 ;
DATAVAL	*dst1, *dst2, *dst3 ;
DATAVAL	*cnst ;
int	*bcnd ;
DATAVAL	*mema ;
int	*delay ;
...
#ifdef	COMMENT
	printf("lexec: opcode:%d src1:%u src2:%u src3:%u src4:%u\n",
		opcode,src1->ui,src2->ui,src3->ui,src4->ui) ;
	printf("lexec: cnst:%u bcnd:%i mema:%u delay:%d\n",
			cnst->ui,*bcnd,mema->ui,delay->ui);
#else
	printf("lexec: opcode:%d src1:%08x src2:%08x src3:%08x src4:%08x\n",
		opcode,src1->ui,src2->ui,src3->ui,src4->ui) ;
	printf("lexec: cnst:%08x bcnd:%d mema:%08x delay:%d\n",
			cnst->ui,*bcnd,mema->ui,*delay);
#endif
...
#if F_DEBUG
		printf("lexec: ADDIU src1=%08x cnst=%08x\n",
			src1->ui,cnst->ui) ;
#endif
...
#if F_DEBUG
		printf("lexec: ADDIU: %x + %x = %x\n",
			src1->ui,cnst->ui,(src1->ui+cnst->ui)) ;
		printf("lexec: dst1=%08x\n", dst1->ui) ;
#endif
...
/* It is only returned if the branch was taken, 1, or not, 0, */
	...
	case LEXECOP_BNE:
#if F_DEBUG
		printf("lexec: BNE src1->i:%d src2->i:%d\n",src1->i,src2->i);
#endif
	    *bcnd = src1->i != src2->i ;
	    *delay = 2 ;
	    break ;
...
/* ilptr = target address */
    ...
    case LEXECOP_JR:
#if F_DEBUG
	printf("lexec:LEXECOP_JR (1) src2->i = %d  "
		"bcnd->i=%d  f_jump=%d\n",
			src2->i, *bcnd, *f_jump);
#endif
...
#if F_DEBUG
	printf("lexec:LEXECOP_JR (2) src2->i = %d  "
		"bcnd->i=%d  f_jump=%d\n",
