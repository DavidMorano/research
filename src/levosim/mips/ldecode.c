...
#define	F_FPRINTF	0		/* fprintf() */
#define	F_SPECIALJR	0
...
/* 01/11/12, Dave Morano, fixed to add second source operand */
...
/* 01/11/12, Dave Morano, removed the second destination operand */
...
/* 01/11/12, Dave Morano, removed second source operand */
...
/* 01/11/12, Dave Morano, added to fix CVTSD and CVTWS */
	case 5:
	    current_instr->src1 = FS_(instr) + OFFSET_1 ;
	    current_instr->src2 = FS_(instr) + OFFSET_1 + 1 ;
	    current_instr->dst1 = FD_(instr) + OFFSET_1 ;
...
#if	F_DEBUGS
	    eprintf("ldecode_fpw: CVTSW\n") ;
#endif

	    break ;

/* 01/11/12, Dave Morano, all messed up ! */
/* single precision float to fixed point */
	case LEVO_EXT_CVTD_OPCODE:
