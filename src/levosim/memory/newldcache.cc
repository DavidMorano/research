...
int ldcache_phase2 (LDCACHE *ldcache)
{
  ...
  if ( rfbus_read(ldcache->MBB, &fgpread) ){
     ...
     if (find == ldcache_not_found){ /* the request is not in the cache */
  ...
  if (rfbus_read(ldcache->MWB, &fgpwrite)){
     ...
     find = is_in_cache(ldcache, addbus_index);
     if (find == ldcache_not_found){ /* the request is a miss */
         struct ldcache_list_entry  request;
...
int ldcache_phase3 (LDCACHE *ldcache)
{
  ...
  static address_index * addbus_index;
  /*static int free;*/
  int addr;
  int i;
  static int number;
	...
	*/
	if ((ldcache -> param.write_policy == write_back) 
	  ...
	  if (! ldmem_busy (ldcache -> ldmem )) {
	    ...
	    ldcache->next_state.wait = ldmem_write (ldcache -> ldmem);
	    addr = ldcache_get_addr(ldcache, ldcache->free, addbus_index->set_value); 
	    for (i = 0; i < ldcache->param.blocksize; i++) 
	      lsim_writeint(ldcache->mip, addr+(i*ldcache->param.interleaved),
  ...
  case 1 : 
    if (ldcache->current_state.wait == 0 ){
      ...
      addr = ldcache->request->address - addbus_index->index;
      for (i = 0; i < ldcache->param.blocksize; i++)
	lsim_readint(ldcache->mip, addr+ (i*ldcache->param.interleaved), 
          &ldcache->way[ldcache->free][addbus_index->set_value].word[i]); 
