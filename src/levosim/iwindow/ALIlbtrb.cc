...
int		bufsize ;
{
	...
	if ((rs = uc_malloc(size,(void **) &lbtrbp->entry)) < 0)
	    assert(0); 
...
/* do the real work ! */
int
lbtrb_assign_predicate(lbtrbp, insttype, instaddr, instTA, pr)
LBTRB   *lbtrbp ;
...
struct lbtrb_predicates *pr;
{
	...
	if(pr->ipaddr == lbtrbp->commited_paddr)
		...
		pr->ipval = -1; /* Branch preidictor would assign the best value */
