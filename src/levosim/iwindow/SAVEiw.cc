...
#define	F_FPRINTF2	0		/* extra fprintf() printing */
...
static int	iw_commitcheckases() ;
static int	iw_commitchecklwq() ;
static int	iw_readyregfilters() ;
static int	iw_getpathcol(), iw_getnextcol() ;
...
/* check it, returns TRUE if it is ready to commit */
	...
	rs = iw_commitcheckases(wp,lip,ci) ;
...
/* OK, is the LWQ ready to take this commitment ? */
	...
	rs = iw_commitchecklwq(wp,lip,ci) ;
...

	{
	    ...
	    for (i = 0 ; (i < lip->totalrows) && (! f_ij) ; i += 1) {
	        ...
	        if (pip->debuglevel > 1) {
	            ...
	            if (rs == SR_EMPTY)
	                eprintf(
                            "iw_commitcheck: row=%d las_commit() rs=EMPTY\n",
				i) ;

	            else
	                eprintf(
                            "iw_commitcheck: row=%d las_commit() rs=%d\n",
				i,rs) ;
...
/* do we have a loaded AS ? */
	        ...
	        if (rs >= 0) {
	            ...
	            f_cfchange = (bdir == 1 && ipval == 1) ;
	            (void) las_commitinfo(wp->lases + 
			(lip->totalrows * ci) + i,
			&asi) ;
...
/* is the instruction a real one or one of the pseudo instructions ? */
	            ...
	            if (asi.iclass > INSTRCLASS_NULL) {
	                ...
	                if (pip->debuglevel > 1)
	                    eprintf(
				"iw_commitcheck: las_commitinfo() ia=%08x\n",
	                        asi.ia) ;
#endif
...
/* send signals back to i-fetch about the committed state of the outcome */
	                        ...
	                        if (pip->debuglevel > 1)
	                            eprintf(
					"iw_commitcheck: lifetch_bu() rs=%d\n",
	                                rs) ;
#endif
...
If we have not seen a GOTO yet, and we have seen a target out of the
window by either target buffer thing (LBTRB or Dave's IEWBTTB), then
mark this as an indication that we have an out-of-window jump and that
it will need a GOTO operation later.
...
/* handle the "do not commit further" case */
	    ...
	    if (f_ij && (i < lip->totalrows)) {

/* commit the delay slot AS */

#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: indirect jump\n") ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: DS las_commit() rs=%d\n",rs) ;
#endif

/* compensate for the delay slot after an indirect jump */

	        n = i + 1 ;
	        for (i = n ; i < lip->totalrows ; i += 1) {

	            rs = las_unused(wp->lases + (lip->totalrows * ci) + i) ;

#if	F_DEBUG
	            if (pip->debuglevel > 1)
	                eprintf("iw_commitcheck: las_unused() rs=%d\n",rs) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: processing GOTO\n") ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: iw_targetinlb() rs=%d\n",rs) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: llb_read_complete() rs=%d\n",
			rs) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: GOTO ta=%08x\n",ta) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: lifetch_goto() rs=%d\n",rs) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: GOTO f_loadlb=%d\n",
			wp->f.loadlb) ;
#endif
...
#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: iw_flushwindow() rs=%d\n",rs) ;
#endif


/* Flush BTRB, we need to build a new set of preidcators */

		rs = lbtrb_flush(&wp->lbtrb) ;

#if	F_DEBUG
	        if (pip->debuglevel > 1)
	            eprintf("iw_commitcheck: lbtrb_flush() rs=%d\n",rs) ;
#endif
...
/* update the "Ali" register file somehow */
	...
	} /* end block (committing the ASes) */


/* commit the LBTRB */
/* must be called after all AS have called lbtrb_as_commit */
...
static int iw_commitcheckases(wp,lip,ci)
IW		*wp ;
struct levoinfo	*lip ;
int		ci ;
{
...
static int iw_commitchecklwq(wp,lip,ci)
IW		*wp ;
struct levoinfo	*lip ;
int		ci ;
{
...
#if	F_FPRINTF
	printf("iw_printregs: committed registers c_ncols=%d rfid=%d rftt=%d\n",
	    ncols,rfid,rfp->c.tt) ;
#endif
...
#if	F_FPRINTF
	    if (rs < 0)
	        fprintf(stdout,"iw_printregs: reg(%3d)=**bad rs=%d**\n",
	            i,rs) ;

	    else if (rs == 0)
	        fprintf(stdout,"iw_printregs: reg(%3d)=%08lx tt=%d\n",
	            i,fg.dv,fg.tt) ;

	    else if (rs == 1)
	        fprintf(stdout,"iw_printregs: reg(%3d)=*invalid*\n",
	            i) ;

	    else if (rs == 2)
	        fprintf(stdout,"iw_printregs: reg(%3d)=*nodata*\n",
	            i) ;
#endif /* F_FPRINTF */
