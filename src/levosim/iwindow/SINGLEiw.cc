...
/* second "do" the one that serves as "bottom" (logically) */

	nsgci = (sgci + 1) % lip->nsgcols ;

	rfti = wp->n.sgcols[nsgci].rfti ;
	rfbi = wp->n.sgcols[nsgci].rfbi ;

/* get out if there are none alive (program error actually !) */

	if ((rfti < 0) && (rfbi < 0))
		return SR_OK ;

/* if there is only one alive, it is ours, pop it */
...
#if	F_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("iw_rfdisable: doing bottom one rfi=%d\n",rfbi) ;
#endif

		rs = regfilter_settt(rfp + rfbi,-1) ;

		wp->n.sgcols[nsgci].rfbi = -1 ;
...
#if	F_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("iw_rfdisable: doing top one rfi=%d\n",rfti) ;
#endif

		rs = regfilter_settt(rfp + rfti,-1) ;

		wp->n.sgcols[nsgci].rfti = -1 ;
