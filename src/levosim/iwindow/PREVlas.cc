...
/* assign the TT */
	...
	lasp->n.path = path;
	lasp->n.ctt = lasp->n.tt / lip->nsgcols ;	/* column time-tag */
...
/* check the Register Forward bus */
	...
	rs = lbusint_done(lasp->rfwbus.outbus) ;
...
/* check the Register Backward bus */
	...
	rs = lbusint_done(lasp->rbwbus.outbus) ;
...
/* check the Predicate Forward bus */
	...
	rs = lbusint_done(lasp->pfwbus.outbus) ;
...
#if	F_DEBUG
	if (pip->debuglevel > 4)
	eprintf("las_readycommit: waiting for MFB\n") ;
#endif
...
#if	F_DEBUG
	if (pip->debuglevel > 4)
	eprintf("las_readycommit: waiting for MBB\n") ;
#endif
...
struct levoinfo	*lip ;
{
	...
	eprintf("las/MBWB_interface: 3 bad sanityfunc LAS=%08lx\n",lasp) ;
		...
		lasp->n.event_table[WAITON_VALUE][MFWB][MEM].valid = 1;
...
/* we snoop the correct memory bus based on our address */
	...
	if (pip->debuglevel > 2)
	    eprintf(
                "las/snoop_MFWB_interface: read a=%08x dv=%08x\n",
			rd.addr,rd.dv) ;
#endif
	...
	if (pip->debuglevel > 2)
	    eprintf(
                "las/snoop_MFWB_interface: match, our_a=%08x\n",
			lasp->c.mem.a) ;
#endif
			...
			if(lasp->c.opclass == INSTR_LOAD) {
				if(rd.trans == LFLOWGROUP_TNULLIFY) {
#if	F_LASDETAILS
					las_print_asid(lasp);
					printf("Received Nulify on MFWB at %d, data from tt=%d\n", lasp->asid, rd.tt);
#endif
					fill_packet(&lasp->n.event_table[RECV_NULLIFY][MFWB][MEM], &rd);
				}
				else {
#if	F_LASDETAILS
					las_print_asid(lasp);
					printf("Snoop on MFWB at %d, data from tt=%d;mem(0x%x)=0x%x\n", lasp->asid, rd.tt,rd.addr,rd.dv);
#endif

					if(lasp->c.opclass == INSTR_LOAD) {
						lasp->n.mem.latesttt = rd.tt;
						boundry = lasp->c.mem.a % 4;
						switch (lasp->c.opexec) {
							...
							case LEXECOP_LB:
								if(boundry == 0) {
									lasp->n.dst1.val = (rd.dv & 0xff000000) >> 24;
									sign = rd.dv & 0x80000000 ? 1 : 0;
								}
								else if(boundry == 1) {
									lasp->n.dst1.val = (rd.dv & 0x00ff0000) >> 16;
									sign = rd.dv & 0x00800000 ? 1 : 0;
								}
								else if(boundry == 2) {
									lasp->n.dst1.val = (rd.dv & 0x0000ff00) >> 8;
									sign = rd.dv & 0x00008000 ? 1 : 0;
								}
								else if(boundry == 3) {
									lasp->n.dst1.val = (rd.dv & 0x000000ff);
							...
							case LEXECOP_LH:
								if(boundry == 0) {
									lasp->n.dst1.val = (rd.dv & 0xffff0000) >> 16;
									sign = rd.dv & 0x80000000 ? 1 : 0;
								}
								else if(boundry == 1) {
									lasp->n.dst1.val = (rd.dv & 0x00ffff00) >> 8;
									sign = rd.dv & 0x00800000 ? 1 : 0;
								}
								else if(boundry == 2) {
									lasp->n.dst1.val = (rd.dv & 0x0000ffff);
									sign = rd.dv & 0x00008000 ? 1 : 0;
								}
								else
							...
							case LEXECOP_LWL:
								if(boundry == 0)
									...
									lasp->n.dst1.val = (lasp->c.src2.val & 0x000000ff) | ((rd.dv & 0x00ffffff) << 8);
								else if(boundry == 2)
									lasp->n.dst1.val = (lasp->c.src2.val & 0x0000ffff) | ((rd.dv & 0x0000ffff) << 16);
								else
									lasp->n.dst1.val = (lasp->c.src2.val & 0x00ffffff) | ((rd.dv & 0x000000ff) << 24);
								break;
							case LEXECOP_LWR:
								if(boundry == 0)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xffffff00) | ((rd.dv & 0xff000000) >> 24);
								else if(boundry == 1)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xffff0000) | ((rd.dv & 0xffff0000) >> 16);
								else if(boundry == 2)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xff000000) | ((rd.dv & 0xffffff00) >> 8);
								else
									lasp->n.dst1.val = rd.dv ;
								break;
							default:
					...
					fill_packet(&lasp->n.event_table[RECV_VALUE][MFWB][MEM], &rd);
				}
			}
		} 
	}
...
/* only the bus with the right interleaving number needs to be snooped */
	...
	if (lbusint_read(busintp, &rd) > 0) {
		...
		if (f_match && lasp->c.opclass == INSTR_STORE) {
			fill_packet(&lasp->n.event_table[RECV_REQ_VALUE][MBWB][MEM], &rd);
#if	F_LASDETAILS
		las_print_asid(lasp);
			printf("Snoop on MBWB at %d, data from tt=%d; mem(0x%x)\n", lasp->asid, rd.tt, rd.addr);
#endif
		}
