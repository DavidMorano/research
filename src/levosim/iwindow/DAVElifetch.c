...
#if	F_FPRINTF
	if (ia == lifp->exitaddr) {
	    printf("lifetch_fetch: Simulation terminated\n") ;
	}
#endif
...
#if	F_MASTERDEBUG && F_DEBUG
	    if (pip->debuglevel > 4)
	        eprintf("lifetch_fetch: PF fetching ia=%08x\n",ia) ;
#endif
...
/* assert(0) ; */
	        ...
	        rs = ldecode_decode(&current_instr) ;
...
#if	F_MASTERDEBUG && F_DEBUG
	        if (pip->debuglevel > 2) {
	            eprintf("lifetch_fetch: bad 1 sanity i=%d rs=%d\n", 
	                lbindex,rs) ;
	        }
#endif
...
/* global sanity */
	    ...
	    if (lifp->sanity.func != NULL) {
	        ...
	        if (current_instr.opclass != INSTR_UNUSED)
...
#if	F_FPRINTF
	        printf(
	            "lifetch_fetch: llb_load() returned negative (rs=%d)\n",
	            rs) ;
#endif
