...
/* we snoop the correct memory bus based on our address */
	...
	if (pip->debuglevel > 2)
	    eprintf(
                "las/snoop_MFWB_interface: read dv=%08x\n",rd.dv) ;
#endif
		...
		if (f_match && rd.tt >= lasp->c.mem.latesttt && rd.tt < lasp->c.tt) {
				...
				if(rd.trans == LFLOWGROUP_TNULLIFY) {
					...
					if(lasp->c.opclass == INSTR_LOAD) {
						...
						switch (lasp->c.opexec) {
							...
							case LEXECOP_LWR:
								if(boundry == 0)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xffffff00) | ((rd.dv >> 24) & 0xff);
								else if(boundry == 1)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xffff0000) | ((rd.dv >> 16) & 0xffff);
								else if(boundry == 2)
									lasp->n.dst1.val = (lasp->c.src2.val & 0xff000000) | ((rd.dv >> 8) & 0xffffff);
								else
									lasp->n.dst1.val = rd.dv ;
								break;
							default:
