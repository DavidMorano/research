...
struct levoinfo	*lip ;
{
	...
	llbp->nentries = bsize = lip->totalrows;

	llbp->readyread = 0;
	llbp->load_enable = 0;

	llbp->c.entry = 
		(struct llb_entry *) malloc(bsize*sizeof(struct llb_entry));
	llbp->n.entry = 
		(struct llb_entry *) malloc(bsize*sizeof(struct llb_entry));

	for (i=0; i<bsize; ++i) {

		llbp->c.entry[i].cpina = (int *) malloc(numpred*sizeof(int));
		assert(llbp->c.entry[i].cpina);
		llbp->c.entry[i].cpinv = (int *) malloc(numpred*sizeof(int));
		assert(llbp->c.entry[i].cpinv);
		llbp->c.entry[i].ncpin= 0;
		llbp->c.entry[i].valid=0;

		llbp->n.entry[i].cpina = (int *) malloc(numpred*sizeof(int));
		assert(llbp->n.entry[i].cpina);
		llbp->n.entry[i].cpinv = (int *) malloc(numpred*sizeof(int));
		assert(llbp->n.entry[i].cpinv);
		llbp->n.entry[i].ncpin= 0;
		llbp->n.entry[i].valid=0;
	}	
...
LLB	*llbp ;
{
	...
	for (i=0;i<llbp->lip->totalrows; i += 1) {
		...
		for(j=0;j<llbp->lip->nprpas;++j) {
			* (llbp->c.entry[i].cpina + j) = * (llbp->n.entry[i].cpina + j);
			* (llbp->c.entry[i].cpinv + j) = * (llbp->n.entry[i].cpina + j);
		}
...
llb_load(llbp, index, faddr, instr,oopexec,oopclass,opexec, opclass, btype,
	...
	const1, const1_valid, pina, cpina, ncpin,pouta,cpouta,pinv,cpinv)
LLB	*llbp;
...
int cpouta;	/* address of cpout reg (-1 means no canceling output predicate) */
