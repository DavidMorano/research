...
int lexec(opcode, src1,src2,src3,src4,dst1,dst2,dst3,cnst,bcnd,mema,delay)
int opcode;
DATAVAL *src1, *src2, *src3, *src4,*dst1, *dst2, *dst3, *cnst, *bcnd, *mema;
int *delay;
{
  int EXCEPTION;             /* EXCEPTION FLAG */  

	int	rs = LEXEC_ROK ;
	int	aux,temp,temp2,conflict=0;              
  ...
  *delay=1;
  ...
  case LEXECOP_MFC1:
    ...
    *delay =2;
    ...
    /* GENERAL ASSUMPTIONS ABOUT FLOATING POINT OPERATIONS:
       ...
       if FGR8 is going to be used for double precision then
       it would be formed by FGR7 (which will fomr the FGR8 LOW part)
       single and FGR8 single (which would conform the HI part)
    */
  case LEXECOP_ABSD:
  ...
  case LEXECOP_LWC1:
    ...
    */
  case LEXECOP_CVTDS:
    /* Convert single precision to double */
#if	F_FUNNYDOUBLE
    dst1->d = (double) src1->f;
#else
	dst1->ui = 0 ;
#endif
    break;
  ...
  case LEXECOP_DIVD:
    ...
    */
  case LEXECOP_MOVD:
      /* AS must access register fs put ts contents on src1->d
	 and then later take the value of dst1->d  and put it in
	 fd */
#if	F_FUNNYDOUBLE
    dst1->d = src1->d;
#else
...
#if	F_FUNNYDOUBLE
    f_isnand = ((isnand(src1->d))||(isnand(src2->d))) ;
#else
	f_isnand = 0 ;
#endif
...
main(){
  ...
  DATAVAL	s1,s2,s3,s4,dest1,dest2;
