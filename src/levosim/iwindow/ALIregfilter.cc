...
#define	F_UPDATEFWD	1		/* forward registers on update ? */
...
#define	F_EXTRASNOOP	1		/* do the extra snoop ? */
...
/**************************************************************************
	...
	**NOTE** currently, we are forwarding all NULLIFYs or other
	...
	entirely free) we are going to try and manage our bus
	...
	straight forward bus requires the allocation and small
	management of the bus read registers.  Writing buses is the
	real problem but we are trying to be cleavor about making
	...
	backwards for forwardings buses.  A 'TRUE' value indicates that
	...
	happens, we try and see if the real data or transaction has
...
static int regfilter_readbuses(REGFILTER *,struct proginfo *,BUS *,int,
				int,int,int,int) ;
static int regfilter_checkgrants(REGFILTER *,struct proginfo *,
				LSIM *) ;
static int regfilter_lookup(REGFILTER *,struct proginfo *,
				struct levoinfo *) ;
static int regfilter_readupdate(REGFILTER *,struct proginfo *,
				struct levoinfo *) ;
...
/* the machine architected registers */
	...
	for (path = 0 ; path < lip->npaths ; path += 1) {
	    ...
	    for (addr = 0 ; addr < REGFILTER_NREGS ; addr += 1) {
	        ...
	        rfp->c.regs[i].fg.addr = rfp->n.regs[i].fg.path = addr ;
...
/* process the data from backwarding buses */
	    ...
	    rs = regfilter_lookup(rfp,pip,lip) ;
...
#if	F_DEBUG && 0
	    if (pip->debuglevel > 4)
	        eprintf("regfilter_comb: regfilter_readupdate()\n") ;
#endif

	    rs = regfilter_readupdate(rfp,pip,lip) ;
...
REGFILTER	*rfp2 ;
{
	...
	if ((rfp->magic != REGFILTER_MAGIC) && (rfp->magic != 0)) {
	    ...
	    eprintf("regfilter_update: bad format\n") ;
...
#if	F_DEBUG
	if (pip->debuglevel > 3) {
	    eprintf("regfilter_update: id=%d rftt=%d\n",
	        rfp->id,rfp->c.tt) ;
	}
#endif
	...
	for (ri = 0 ; ri < rfp->nregs ; ri += 1) {
	    ...
	    if (f_invalid ||
	        ...
	        if (pip->debuglevel > 3) {
	            ...
	            eprintf(
	                "regfilter_update: new p=%d tt=%4d a=%4d dv=%08x\n",
	                rfp2->c.regs[ri].fg.path,
	                rfp2->c.regs[ri].fg.tt ,
...
#if	F_UPDATEFWD
	        rfp->n.regs[ri].fage = rfp->c.regs[ri].fage + 1 ;
#else
	        rfp->n.regs[ri].fage = 0 ;
#endif
...
#if	F_DEBUG
	if (pip->debuglevel > 1) {
	    eprintf("regfilter_writebuses: rftt=%d f_fownbus=%d f_fwdv=%d\n",
	        rfp->c.tt,
	...
	if (pip->debuglevel > 1)
	    ...
	    if (f_gotone) {
	        ...
	        if (pip->debuglevel > 1) {
	            ...
	            eprintf(
	                "regfilter_writebuses: rftt=%d F writing BUS=%08lx\n",
...
#if	F_DEBUG || F_FPRINTF
	    mkttbuf(ttbuf,rfp->c.bwd.fg.tt) ;
#endif

#if	F_DEBUG
	    if (pip->debuglevel > 1) {
	        eprintf(
	            "regfilter_writebuses: B writing tt=%s:%d a=%d "
	            "dp=%d dv=%08x\n",
	            ttbuf,
	            fgp->seq,
	            rfp->c.bwd.fg.addr,
...
static int regfilter_lookup(rfp,pip,lip)
REGFILTER	*rfp ;
struct proginfo	*pip ;
struct levoinfo	*lip ;
{
...
#if	F_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("regfilter_lookup: entered\n") ;
#endif
...
/* do a sanity check on the path */
	        ...
	        if ((fgp->path < 0) || (fgp->path >= lip->npaths)) {
...
#if	F_DEBUG
	            if (pip->debuglevel > 1)
	                eprintf("regfilter_lookup: reg invalid\n") ;
#endif
...
/****
	...
	NULLIFYs are a whole new ball-game and that they work
	nothing like one would imagine !  Everything is off.

****/

static int regfilter_readupdate(rfp,pip,lip)
REGFILTER	*rfp ;
struct proginfo	*pip ;
struct levoinfo	*lip ;
{
...
#if	F_DEBUG
	if (pip->debuglevel > 1)
	    eprintf("regfilter_readupdate: entered id=%d rftt=%d\n",
	        rfp->id,rfp->c.tt) ;
#endif
...
/* remove the structurally improper transactions */
	...
	for (i = 0 ; i < n ; i += 1) {
...
#if	F_DEBUG
	    if (pip->debuglevel > 1)
	        eprintf("regfilter_readupdate: id=%d rftt=%d P loop_i=%d\n",
	            rfp->id,rfp->c.tt,i) ;
#endif
...
#if	F_DEBUG
	    if ((pip->debuglevel > 1) && rfp->brr_v[i]) {

	        eprintf(
	            "regfilter_readupdate: ftt=%d "
	            "tr=%d p=%d tt=%s:%d a=%d "
	            "dp=%d dv=%08x\n",
	            fgp->ftt,
	            fgp->trans,
	    ...
	    if (rfp->brr_v[i]) {
	        ...
	        fprintf(stdout,
	            "regfilter_readupdate: id=%d rftt=%d tr=%d p=%d "
	            "tt=%s:%d a=%d "
	            "dp=%d dv=%08x\n",
	            rfp->id,rfp->c.tt,
	            fgp->trans,
	                ...
	                if ((tfgp->path == path) &&
	                    ...
	                    if (j != besti) {
	                        ...
	                        if (pip->debuglevel > 1) {
	                            ...
	                            eprintf(
	                                "regfilter_readupdate: W=%d ftt=%d "
	                                "tr=%d p=%d tt=%s:%d a=%d "
	                                "dp=%d dv=%08x\n",
	                                j,
	                                tfgp->ftt,
...
#if	F_DEBUG
	    if (pip->debuglevel > 1)
	        eprintf("regfilter_readupdate: id=%d rftt=%d loop_i=%d\n",
	            rfp->id,rfp->c.tt,i) ;
#endif
...
#if	F_DEBUG
	            if (pip->debuglevel > 1)
	                eprintf("regfilter_readupdate: bad path=%d\n",
	                    fgp->path) ;
#endif /* F_DEBUG */
...
#if	F_DEBUG
	            if (pip->debuglevel > 1)
	                eprintf("regfilter_readupdate: bad addr=%d\n",
	                    fgp->addr) ;
#endif /* F_DEBUG */
...
/* calculate some flags for later use */
	        ...
	        if (pip->debuglevel > 1) {
	            eprintf(
	                "regfilter_readupdate: f_invalid=%d "
	                "f_brout=%d frseq=%d\n",
	                f_invalid,rp->f.brout,rp->frseq) ;
	            eprintf(
	                "regfilter_readupdate: existing tt=%d:%d "
	                "dp=%d dv=%08x\n",
	                rfp->n.regs[ri].fg.tt,
...
/* we currently have invalid data */
	                ...
	                if (pip->debuglevel > 1)
	                    eprintf(
	                        "regfilter_readupdate: NULL w/ invalid\n") ;
#endif
	                ...
	                if (f) {
	                    ...
	                    if (pip->debuglevel > 1)
	                        eprintf(
	                            "regfilter_readupdate: updating TT\n") ;
#endif
...
/* we have valid data, invalidate us only if the TTs match exactly ! */
	                ...
	                if (pip->debuglevel > 1)
	                    eprintf(
	                        "regfilter_readupdate: NULL w/ valid"
	                        " old_tt=%d new_tt=%d\n",
	                        rp->fg.tt,fgp->tt) ;
#endif
	                ...
	                if (f) {
	                    ...
	                    if (pip->debuglevel > 1)
	                        eprintf(
	                            "regfilter_readupdate: invalidate\n") ;
#endif
...
/* cancel any forwarding that may have been in progress */
	                    ...
	                    if (pip->debuglevel > 1)
	                        eprintf(
	                            "regfilter_readupdate: f_fwd=%d fage=%d\n",
	                    ...
	                    if (rp->f.forward || (rp->fage > 0)) {
	                        ...
	                        if (pip->debuglevel > 1)
	                            eprintf(
	                                "regfilter_readupdate: cancel FWD\n") ;
#endif

	                        rp->f.forward = FALSE ;
	                        rp->fage = 0 ;
	                        f_request = TRUE ;	/* special request */
...
/* forward if necessary */
			...
			int	f_prenull ;
	                ...
	                if (pip->debuglevel > 1)
	                    eprintf(
	                        "regfilter_readupdate: need to forward\n") ;
#endif
...
/* shift in the value that needs forwarding first */
	                    ...
	                    if (pip->debuglevel > 1)
	                        eprintf(
	                            "regfilter_readupdate: S => FIFO tr=%d p=%d"
...
/* finally, schedule the forwarding of the NULLIFY */
	                ...
	                if (pip->debuglevel > 1)
	                    eprintf(
	                        "regfilter_readupdate: N => FIFO tr=%d p=%d"
	                        " tt=%d a=%d dp=%d dv=%08x\n",
	                        fgp->trans,fgp->path,
	                        fgp->tt, fgp->addr,
	                        (fgp->dp != 0),fgp->dv) ;
	                ...
	                if (pip->debuglevel > 2)
	                    eprintf(
	                        "regfilter_readupdate: lfgf_write() rs=%d\n",
	                        rs) ;
#endif
...
/* do we need a backward ? */
	            ...
	            if ((f_killed || f_greater) && 
	                (rfp->c.tt > 0) && 
	                (f_request || ((rfp->c.tt % lip->totalrows) == 0))) {

#if	F_DEBUG
	                if (pip->debuglevel > 2)
	                    eprintf(
	                        "regfilter_readupdate: need BWD\n") ;
#endif
	            ...
	            if (f_invalid) {
	                ...
	                if (fgp->tt == rp->fg.tt) {
	                    ...
	                    if (pip->debuglevel > 2)
	                        eprintf(
	                            "regfilter_readupdate: extrasnoop f=%d\n",
	                            f) ;
#endif
	            ...
	            if (f) {
	                ...
	                if (pip->debuglevel > 1) {
	                    eprintf(
	                        "regfilter_readupdate: updating TT\n") ;
	                    eprintf(
	                        "regfilter_readupdate: a=%d dp=%d dv=%08x\n",
	                        fgp->addr,
	                        ((fgp->dp) ? 1 : 0),
	                        fgp->dv) ;
	                }
...
/* do we have a data change ? */
	                ...
	                if (f_invalid || f_datachange || rp->f.brout) {
	                    ...
	                    if (pip->debuglevel > 1)
	                        eprintf(
	                            "regfilter_readupdate: updating register"
	                            " f_datachange=%d \n",
	                            f_datachange) ;
#endif
...
/* should we forward ? */
	...
	if (pip->debuglevel > 1)
	    eprintf(
	        "regfilter_readupdate: exiting rs=%d "
...
/* end subroutine (regfilter_readupdate) */
...
struct proginfo	*pip ;
{
	...
	if (pip->debuglevel > 1)
	    eprintf(
	        "regfilter_requests: rftt=%d f_fwdregs=%d f_fwdfifo=%d "
	        "f_bwd=%d\n",
...
/* check the RFB FIFOs */
	...
	if (pip->debuglevel > 3)
	    eprintf(
		"regfilter_checkholds: F FIFO=%i "
		"entries remaining=%d\n",
	        i,cr) ;
#endif

		if (cr < REGFILTER_MINFREE) {
