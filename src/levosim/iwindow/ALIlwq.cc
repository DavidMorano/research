...
#define	F_FORCE100	1		/* force 100 for depth ? */
...
/* save the pointers to the buses that go to the memory subsystem */
	...
	nmb = lwqp->nmb = lwqp->lip->nmembuses ;
...
/* initialize the LBUSINT objects */
	...
	for (i=0; i<lwqp->norows; ++i) {
	    ...
	    lwqp->c.as_fifo[i].valid=0 ;
	    lwqp->n.as_fifo[i].valid=0 ;
	}
...
/*
	for(i=0; i<lwqp->norows; ++i) {
	    ...
	    j=lsq_lookup(&lwqp->lsqes[i],addr,datap,ttp) ;
	    if(j>=0) {
	        if(*ttp > mtt) {
	            data = *datap ;
	            mtt = *ttp ;
	...
	eprintf("lwq_snoop_mbwb: lbusint_read() LBUSINT=%08lx\n",
	    ...
	    if (lbusint_read(busintp, &rd) > 0) {
	        ...
	        if(wqp->f_qfull)
	            ...
	            printf("snooped at WQ: Mem[0x%x],tt=%d, data=0x%x\n",
				rd.addr,rd.tt,rd.dv) ;
	...
	do {
	    ...
	    if (wqp -> en[wqp->qr].ready) {
	        ...
	        if(ret >= 0) {
	            ...
	            printf("Unsuccessful sending data on MFWB "
			"at WQ :Mem(0x%x)=%d\n",
			wrp->addr,wrp->dv) ;
	        }
