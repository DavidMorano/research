...
int lwq_init(lwqp,pip,mip,lip, ap)
LWQ             *lwqp ;
struct proginfo *pip ;
LSIM	 	*mip ;
struct levoinfo *lip ;
...
/* make sure that data with lower tt is written first */
	    ...
	    for (row=0; row < lwqp->norows ; row += 1) {
...
#else
		if (tt1 >= ep->tt)
		printf("WQ: TT of write is large wtt=%d\n",ep->tt) ;
#endif
	...
	if (lsim_writeint(lwqp->mip,ep->addr, ep->data,ep->dp) != SR_OK) {
		...
		printf("WQ: unsuccessful Write to "
			"memory(0x%x),dp=0x%x, data=0x%x\n",
...
int lwq_store(lwqp, addr, data, tt, row, dp)
LWQ	*lwqp ;
uint addr;	/* address */
int data;	/* data */
int tt;		/* time tag */
...
Does a lookup in all the internal store queues and returns the latest
value Returns the index of the lsq that has the lastes value, -1 if
couldn't find a match

*/

int lwq_lookup(lwqp, addr, datap, ttp)
LWQ	*lwqp ;
uint	addr ;
int	*datap ;
int	*ttp ;
...
static int lwq_snoop_mbwb(lwqp)
LWQ	*lwqp ;
{
	struct proginfo	*pip ;
