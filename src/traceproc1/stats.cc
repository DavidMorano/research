...
#define	CF_SAFE2	1		/* extra safe mode? */
#define CF_PVALUESBEF	1		/* print values before */
#define CF_PVALUESAFT	0 		/* print values after */
#define CF_CREATETRACE	0		/* create debugging trace data */
#define	CF_MEMFAULT	0		/* fault on memory */
#define	CF_EXECFAULT	0		/* fault on instruction execution */
#define	CF_SSHTESTING	0		/* SSH testing */
#define	CF_BREAKVP	0		/* break the value-prediction */
...
#define	FE_MEMRINT	".mrint"
#define	FE_MEMLIFE	".mlife"
#define	FE_MEMUSE	".muse"
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: i=%d tourna_init()\n",i) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: tourna_init() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: i=%d gskew_init()\n",i) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: gskew_init() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("stats: bpeval_init() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: loadbps() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("stats: fcount_init() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("stats: opt_regstats\n") ;
#endif

	    rs = regstats_init(&rstats,-1,-1,256) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("stats: regstats_init() rs=%d\n",rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("stats: memstats_init() rs=%d\n",rs) ;
#endif
...
/* initial instruction address */
	...
	rs = lmapprog_getentry(mpp,&uiw) ;

	smp->ia = uiw ;

/* stack pointer */

	rs = lmapprog_getstack(mpp,&uiw) ;

	smp->regs[LMIPSREG_SP] = uiw ;


/* other instruction tracking */
...
/* TRACE: read the trace (get the next instruction) */
	    ...
	    while (TRUE) {
	        ...
	        if ((pip->debuglevel >= 2) && (rs < 0))
	            debugprintf("stats: exectrace_read() rs=%d tr=%llu in=%llu\n",
	                rs,trecs,smp->in) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("stats: end-of-trace tr=%llu in=%llu ia=%08x\n",
	                trecs,smp->in,ia_next) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(3))
	            debugprintf("stats: bad IA in=%llu "
			"next_ia=%08x e_ia=%08x\n",
	                smp->in,
	                ia_next,e.ia) ;
#endif
...
/* update any branch predictors */
	    ...
	    if (bii.f_cbr) {
	        ...
	        if (f_tourna) {
	            ...
	            if (f_se && LEQUIV(bii.f_tourna,f_outcome))
	        ...
	        if (f_gskew) {
	            ...
	            if (f_se && LEQUIV(bii.f_gskew,f_outcome))
...
#if	CF_MASTERDEBUG && CF_DEBUG && CF_CREATETRACE
	    if (DEBUGLEVEL(5) &&
	        (instrdis[0] == '\0')) {
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(2))
	            debugprintf("stats: proginfo_selection() rs=%d\n",
	                rs) ;
#endif
	        return rs ;
	    }
...
/* prepare some operand stuff */
	    ...
	    if (pip->f.opt_regstats) {
	        ...
	        if (di.src1 >= 0)
	            regstats_read(&rstats,(uint) smp->in,f_se,
...
/* we handle load/stores ourselves */
...
#if	CF_MASTERDEBUG && CF_DEBUG && CF_PVALUESBEF
	            if (DEBUGLEVEL(5)) {
	            ...
	            if (pip->f.opt_memstats) {
	                ...
	                memstats_read(&mstats,(uint) smp->in,f_se,ma,0) ;

	                if (di.opexec == LEXECOP_LDC1)
	                    memstats_read(&mstats,(uint) smp->in,
					f_se,(ma + 4),0) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG && CF_PVALUESAFT
	            if (DEBUGLEVEL(5)) {
	            ...
	            if (pip->f.opt_memstats) {
	                ...
	                memstats_write(&mstats,(uint) smp->in,f_se,ma,0) ;

	                if (di.opexec == LEXECOP_SDC1)
	                    memstats_write(&mstats,(uint) smp->in,
				f_se,(ma + 4),0) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG && CF_MEMFAULT
	            if ((pip->debuglevel >= 2) && (rs < 0))
...
#if	CF_MASTERDEBUG && CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("stats: in=%lld yags_lu() rs=%d\n",
	                            smp->in,rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("stats: in=%lld tourna_lu() rs=%d\n",
	                            smp->in,rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("stats: in=%lld gspag_lu() rs=%d\n",
	                            smp->in,rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	                    if (DEBUGLEVEL(4))
	                        debugprintf("stats: in=%lld gskew_lu() rs=%d\n",
	                            smp->in,rs) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG && CF_EXECFAULT
	            if ((pip->debuglevel >= 2) && (rs < 0)) {
	                debugprintf("stats: lexec() rs=%d\n",rs) ;
	                debugprintf("stats: in=%lld ia=%08x instr=%08x\n",
	                    smp->in,ia,instr) ;
...
/* accumulate register statistics */
...
/* reads */
	            ...
	            if (di.src1 >= 0)
	                regstats_readupdate(&rstats,(uint) smp->in,f_se,
	                    di.src1,smp->regs[di.src1]) ;
...
/* writes */
	            ...
	            if (di.dst1 > 0)
	                regstats_write(&rstats,(uint) smp->in,f_se,
	                    di.dst1,smp->regs[di.dst1]) ;
...
/* update any predictors that have scheduled updates pending */
	    ...
	    if (f_yags) {
	        ...
	        rs = bpfifo_read(&bf_yags,&fin,&lia,&frow,&foutcome) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	            if (DEBUGLEVEL(4)) {
	                debugprintf("stats: in=%lld tourna_update() \n",
	                    smp->in) ;
	                debugprintf("stats: frow=%d fia=%08x\n",
	                    frow,fia) ;
	            }
...
#if	CF_MASTERDEBUG && CF_DEBUG
	            if (DEBUGLEVEL(4)) {
	                debugprintf("stats: in=%lld gskew_update() \n",
	                    smp->in) ;
	                debugprintf("stats: frow=%d fia=%08x\n",
	                    frow,fia) ;
	            }
...
#if	CF_MASTERDEBUG && CF_DEBUG
	            if (DEBUGLEVEL(4))
	                debugprintf("stats: in=%lld gskew_update() rs=%d\n",
	                    smp->in,rs) ;
#endif
...
/* finish up */
	...
	if (pip->f.opt_regstats)
	    regstats_writedone(&rstats,(uint) smp->in,f_se) ;

	if (pip->f.opt_memstats)
	    memstats_writedone(&mstats,(uint) smp->in,f_se) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats: gskew tlen=%d bits=%d\n",
	                s.tlen,s.bits) ;
#endif
...
/* function instruction counts */
	    ...
	    if (pip->f.opt_fcount) {
	        ...
	        if ((rs1 = bopen(&tmpfile,tmpfname,"wct",0666)) >= 0) {
	            ...
	            fcount_gettotal(&funcs,&ins,NULL) ;

	            bprintf(&tmpfile,"%12lu %7.3f%% %08x %08x %s\n",
	                ins,100.0,0,0,"*total*") ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4)) {
	            debugprintf("stats: tmp1fname=%s\n",tmp1fname) ;
	            debugprintf("stats: tmp2fname=%s\n",tmp2fname) ;
	            debugprintf("stats: tmp3fname=%s\n",tmp3fname) ;
	            debugprintf("stats: tmp4fname=%s\n",tmp4fname) ;
	            debugprintf("stats: tmp5fname=%s\n",tmp5fname) ;
	        }
#endif /* CF_DEBUG */
...
/* read-interval */
	            ...
	            bprintf(&tmpfile,"read-interval mean        %17.4f\n",
	                st.rint_mean) ;
...
/* useful-lifetime */
	            ...
	            bprintf(&tmpfile,"lifetime mean             %17.4f\n",
	                st.life_mean) ;
...
/* def-use */
	            ...
	            bprintf(&tmpfile,"def-use mean              %17.4f\n",
	                st.use_mean) ;
...
/* write to the stats file */
	        ...
	        if ((rs1 = bopen(&tmpfile,tmp1fname,"wa",0666)) >= 0) {
	            ...
	            memstats_stats(&mstats,&st) ;

	            bprintf(&tmpfile,"= memory statistics\n") ;

	            bprintf(&tmpfile,"memory reads              %12u\n",
	                st.reads) ;

	            bprintf(&tmpfile,"memory writes             %12u\n",
	                st.writes) ;
...
/* read-interval */
	            ...
	            bprintf(&tmpfile,"1k read-interval mean     %17.4f\n",
	                st.rint1_mean) ;
...
/* useful-lifetime */
	            ...
	            bprintf(&tmpfile,"1k lifetime mean          %17.4f\n",
	                st.life1_mean) ;
...
/* def-use */
	            ...
	            bprintf(&tmpfile,"1k def-use mean           %17.4f\n",
	                st.use1_mean) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	if (DEBUGLEVEL(4))
	    debugprintf("stats: returning in=%llu rs=%d\n",smp->in,rs) ;
#endif


bad5:
	if (pip->f.opt_memstats)
	    memstats_free(&mstats) ;
...
#if	CF_MASTERDEBUG && CF_DEBUG
	        if (DEBUGLEVEL(4))
	            debugprintf("stats/writestats: dynamic preds\n") ;
#endif
...
/* do we support this option? */
	    ...
	    if ((oi = matostr(keyopts,2,kp,klen)) >= 0) {
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("loadbps: p1=%d p2=%d p3=%d p4=%d\n",
			p1,p2,p3,p4) ;
#endif
...
#if	CF_MASTERDEBUG && CF_DEBUG
	    if (DEBUGLEVEL(4))
	        debugprintf("loadbps: bpeval_add() rs=%d\n",rs) ;
#endif
