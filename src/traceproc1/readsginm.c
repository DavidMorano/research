...
#if	CF_DEBUGS
	    debugprintf("readsginm: got one >%t<\n",sp,(len -1)) ;
#endif

	    memset(&r,0,sizeof(struct record)) ;

	    if ((rs = field_start(&lf,sp,len)) >= 0) {
...
#if	CF_DEBUGS
	    debugprintf("readsginm: type field=>%t<\n",fp,fl) ;
#endif
...
#if	CF_DEBUGS
	        debugprintf("readsginm: name field=>%t<\n",fp,fl) ;
#endif
