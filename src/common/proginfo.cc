...
static int	proginfo_procopts(struct proginfo *, cchar *,int) ;

static int	setit(void *,cvoid *) ;

local void	freeit(void *) ;
...
int proginfo_init(proginfo *pip,mainv envv,cchar *ap,cchar *version) noex {
...
cchar	ap[] ;
{
	...
	pip->progdname = nullptr ;
	pip->progname = nullptr ;
	if (ap == nullptr)
	    return SR_FAULT ;
	...
	while (field_get(&fsb,oterms) >= 0) {
	    ...
	    case select_in:
	        if (cp2 != nullptr) {
	            ...
	            if ((cp3 = strnchr(cp2,cl2,':')) != nullptr) {
	...
	while (field_get(&fsb,oterms) >= 0) {
	    ...
	    case opt_sin:
	        if (cp2 != nullptr) {
	            ...
	            if ((cp3 = strnchr(cp2,sl2,':')) != nullptr) {
...
static int setit(void *ap,cvoid *vp) noex {
	caddr_t	*p = (caddr_t *) ap ;
	if (vp == nullptr) return SR_OK ;
	if (*p != nullptr) free(*p) ;
	*p = mallocstr(vp) ;
	return (*p != nullptr) ? SR_OK : SR_FAULT ;
}
/* end subroutine (setit) */

/* free up a malloc'ed thing */
local void freeit(void *ap) noex {
	caddr_t	*p = (caddr_t *) ap ;
	if (*p != nullptr) {
	    free(*p) ;
	    *p = nullptr ;
