...
extern int	optmatch(char * const *,char *,int) ;
...
extern int	convert(struct proginfo *,char *,char *,uint) ;
...
/* maximun number of instructions to convert */
				case 'n':
	                            if (argr <= 0)
	                                goto badargnum ;
...
/* option parsing */
	...
	rs = convert(pip,t1fname,t2fname,n) ;
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("main: exiting convert() rs=%d\n",rs) ;
#endif
...
struct proginfo	*pip ;
{
	...
	rs = SR_OK ;
	bprintf(pip->efp,
	    "%s: USAGE> %s pixie_file outtrace [-v]\n",
