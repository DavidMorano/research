...
trace format.  Although the Levo machine being sequenced is
...
registered state information from a Levo machine.  More information
on the Levo machine and its design philosophy can be found in
the Technical Report by Uht.
.\"_
.H 2 "LevoSim"
.P
...
to form the whole machine.  The whole machine is somewhat hierarchical
and this hierarchy is usually expressed within the simulator also
with its various software pieces.  For most of the whole Levo machine,
...
many state machines.  More information on the LevoSim simulator
...
structuring of all machine state data.  There will be two files
produced by LevoSim for the output state trace.  One file will
be the XML proper.  The other file will be an index of the XML
file.  The index file is an index of the start of clock blocks
in the XML file.  The index file contains both the starting offset
...
for relatively modest information content.  Further, closing
tags are really redundant but take up space none-the-less.
...
associated with those tags.  Tags may introduce blocks
...
will be described individually.  Leaf tags will always
...
It currently only has two sub-tags.  Tags that can appear in this
tag block are :
...
These tags are divided into two major types.  The block tags used
...
Levo machine.  The sub-tags for this tag defined so far so far are :
...
This is a big one !  This contains most of the machine
components of the machine.  The sub-tags of this block are :
...
hardware component.  This is basically just a register of
decoded instructions.  The sub-tag for this block are :
...
\fIvalid\fP bit is set in this entry or not
.LI used
a leaf tag containing a single binary digit indicating whether the
...
.H 4 "the sg tag"
.P
This block tag introduces a Sharing Group (SG).  A Sharing Group
...
programmed to output state but some are.  The sub-tags within this
block tag are :
...
.H 4 "the aselem tag"
.P
This is a block tag that exists with Sharing Groups.  This
contains a single 
...
component.  There are often many of these in any configured Levo machine.
...
currently very similar.  A source register set contains the
...
loaded in the AS.  The term 'destination' may be misleading as
this block contains state information for the input of this
register to the current AS.  The values in a destination register
set are those for the operand before any possible computation
by the current AS.  Therefor both source and destination register
...
Element hardware component.  Currently, there is no clearly defined
...
These tags are used in more than one place.  That is, they may be
enclosed by more than one other block tag.
.\"_
.H 4 "the busgroup tag"
.P
This tag introduces a block that describes a group of
related buses.  Some of the sub-tags of this block tag
are optional.  Usually only one other block tag will be
...
hardware object.  This object generally corresponds with a "transaction"
in the Levo machine.  Transactions contain both architectural data
