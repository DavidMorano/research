...
{\it time tags}.  Time tags serve as the basic ordering enforcement mechanism
...
is called the {\it Execution Window}.
...
the operand should be {\it snarfed}~\footnote{snarfing entails snooping
...
to as a {\it transaction}, and consists of :
%
\begin{itemize}
\item{a path ID}
...
{\it time-tag},
{\it address}, and
{\it value} registers are reloaded with new values on each snarf,
while the
{\it path} and
{\it AS time-tag} are only loaded when the active station is
...
\caption{{\it Active Station Source Operand.} The registers and snooping
